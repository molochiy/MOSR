...
namespace IncludingMonotonousApproximationMethod
{
    ...
    {
        public List<double> LeftApproximation { get; set; }
        public List<double> RightApproximation { get; set; }
    }
    /// <summary>
    ...
    public partial class MainWindow : Window
    {
        ...
        private void CalculateButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            var eigenValues = SolveIncludingMonotonousApproximationMethod(initialСonditions);
        ...
        private double GetDeltaLambdaMu(Decomposition decomposition)
        {
            double sum = 0;
        ...
        private double GetDeltaLambdaNu(Decomposition decomposition)
        {
            double sum = 0;
            double sum2 = 0;
            ...
            {
                sum += decomposition.V[i, i] / decomposition.U[i, i];
                sum2 += Math.Pow(decomposition.V[i, i] / decomposition.U[i, i], 2) - decomposition.W[i, i] / decomposition.U[i, i];
            }
        ...
        private Result SolveIncludingMonotonousApproximationMethod(InitialСonditions initialСonditions)
        {
            var eigenValues = new Result { LeftApproximation = new List<double>(), RightApproximation = new List<double>() };
            //eigenValues.RightApproximation.Add(initialСonditions.L0);

            var xPartitions = GetPartitionForIntegrationLimit(initialСonditions);

            var prevEigenValue = initialСonditions.L0;
            ...
            do
            {
                ...
                var deltaLambdaMu = GetDeltaLambdaMu(decomposition);
                var deltaLambdaNu = GetDeltaLambdaNu(decomposition);

                eigenValues.LeftApproximation.Add(prevEigenValue - deltaLambdaMu);
                eigenValues.RightApproximation.Add(prevEigenValue - deltaLambdaNu);

                prevEigenValue = eigenValues.RightApproximation.Last();

            } while (Math.Abs(eigenValues.RightApproximation.Last() - eigenValues.LeftApproximation.Last()) > initialСonditions.Eps);

            return eigenValues;
        }

        private void AddRowToEigenValuesTable(Result eigenValues)
        {
            var eigenValue = eigenValues.LeftApproximation.Last();

            List<string> leftApproximation = eigenValues.LeftApproximation.Select(x => x.ToString()).ToList();
            List<string> rightApproximation = eigenValues.RightApproximation.Select(x => x.ToString()).ToList();
            ...
            var resultsForTable = eigenValueList.Select((x, i) => new { N = i + 1, Mu = leftApproximation[i], Nu = rightApproximation[i], ApproximatedEigenValue = eigenValueList[i] });
