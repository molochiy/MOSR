...
namespace HalleyBilateralFirstApproachApproximation
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void CalculateButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            var eigenValues = SolveBilateralFirstApproachMethod(initialСonditions);
        ...
        private Result SolveBilateralFirstApproachMethod(InitialСonditions initialСonditions)
        {
            ...
            do
            {
                ...
                var nextEigenValue = prevEigenValue - HalleyRelation(decomposition);

                double mu;
                if (determinant * determinantSecondDerivative > 0.0)
                {
                    mu = prevEigenValue - determinant / determinantDerivative;
                }
                else
                {
                    mu = prevEigenValue - GetDeltaLambdaMu(determinant, determinantDerivative, determinantSecondDerivative);
                }


                eigenValues.LeftApproximation.Add(mu);
                eigenValues.RightApproximation.Add(nextEigenValue);
        ...
        private void AddRowToEigenValuesTable(Result eigenValues)
        {
            ...
            var resultsForTable = eigenValueList.Select((x, i) => new { N = i + 1, Mu = leftApproximation[i], Lambda = rightApproximation[i], ApproximatedEigenValue = eigenValueList[i] });
