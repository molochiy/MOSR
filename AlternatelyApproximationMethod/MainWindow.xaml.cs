...
namespace AlternatelyApproximationMethod
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void CalculateButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            var eigenValues = AlternatelyApproximationMethod(initialСonditions);
        ...
        private List<double> AlternatelyApproximationMethod(InitialСonditions initialСonditions)
        {
            ...
            do
            {
                ...
                var deltaLambda = eigenValues.Count.IsEven() ? GetDeltaLambdaNu(decomposition) : GetDeltaLambdaMu(decomposition);
        ...
        {
            var resultsForTable = new List<ResultForTable>();

            var eigenValue = eigenValues.Last();

            resultsForTable = eigenValues.Select((x, i) => new ResultForTable { ApproximatedEigenValue = x == eigenValue ? x.ToString() : "", N = i.ToString(), LeftApproximation = x < eigenValue ? x.ToString() : "", RightApproximation = x > eigenValue ? x.ToString() : "" }).ToList();

            //resultsForTable[resultsForTable.Count - 1].ApproximatedEigenValue = eigenValues.Last().ToString();
